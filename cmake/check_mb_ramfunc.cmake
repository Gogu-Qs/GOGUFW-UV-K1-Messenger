...
string(REGEX MATCH
    "[ \t]\\.mb_ramfunc[ \t]+([0-9A-Fa-f]+)[ \t]+([0-9A-Fa-f]+)[ \t]+([0-9A-Fa-f]+)"
...
set(section_load_hex "${CMAKE_MATCH_3}")
math(EXPR section_size "0x${section_size_hex}")
math(EXPR section_start "0x${section_start_hex}")
math(EXPR section_load "0x${section_load_hex}")
math(EXPR section_end "${section_start} + ${section_size}")
math(EXPR flash_used "${section_load} + ${section_size} - 0x08002800")
math(EXPR flash_free "0x0001D800 - ${flash_used}")
math(EXPR flash_tenths "${flash_used} * 1000 / 0x0001D800")
math(EXPR flash_percent "${flash_tenths} / 10")
math(EXPR flash_decimal "${flash_tenths} % 10")
...
message(STATUS
    ...
    "actual FLASH ${flash_used}/120832 bytes (${flash_percent}.${flash_decimal}%), ${flash_free} bytes free")
