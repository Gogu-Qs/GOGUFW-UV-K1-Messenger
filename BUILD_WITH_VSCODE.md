# Build GOGUFW 2.0.5 with VS Code and Docker

Docker is enough for the firmware build. VS Code is only a convenient front-end.

## Install once

1. Install **Visual Studio Code**.
2. Install and start **Docker Desktop**.
3. In VS Code, install these extensions when prompted:
   - C/C++ by Microsoft
   - CMake Tools by Microsoft

## Open the project

1. Unzip this firmware package.
2. Open VS Code.
3. Use **File > Open Folder...** and select the folder that contains `compile-with-docker.sh`.

## Build

Press:

```text
Cmd + Shift + B
```

Then choose:

```text
GGFW: Build
```

The command run by the task is:

```bash
./compile-with-docker.sh
```

## Clean build

Use:

```text
GGFW: Clean + Build
```

## Output files

After a successful build, run:

```text
GGFW: Show output files
```

The CMake preset is still internally named `Fusion` because GOGUFW is based on the F4HWN Fusion feature set. The firmware branding/version shown on the radio is `GOGUFW v2.0.5 / GGFW`.
