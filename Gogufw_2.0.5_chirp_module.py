# GOGUFW UV-K1 / UV-K5 V3 Messenger CHIRP module v2.0.5
...
FIRMWARE_VERSION_UPDATE = "https://github.com/Gogu-Qs/GOGUFW-UV-K1-Messenger/releases"
...
GGFW_MSG_LED_LIST = ["OFF", "GREEN", "YELLOW"]
...
# advanced settings too
    ...
    def get_memory(self, number):
        ...
        if is_empty:
            ...
            rs = RadioSetting("noFskTx", "No FSK TX", val)
            mem.extra.append(rs)
...
#                mem.extra.append(rs)
        ...
        # No FSK TX
        # No FSK TX shares the otherwise-unused high bit of the step byte.
        # Historical channel images have this bit clear. An erased 0xFF byte
        # remains the firmware's default sentinel and must display as Off.
        step_byte = int(_mem.get_raw()[14])
        no_fsk_tx = step_byte != 0xFF and bool(int(_mem.noFskTx))
        val = RadioSettingValueBoolean(no_fsk_tx)
        rs = RadioSetting("noFskTx", "No FSK TX", val)
        rs.set_doc('Blocks Messenger and Range Check FSK transmissions on this channel. Voice TX and FSK reception remain enabled.')
        mem.extra.append(rs)

        # No Roger shares bit 6 of the step byte. Erased legacy records
        # remain Off, matching the firmware's 0xFF default handling.
        no_roger = step_byte != 0xFF and bool(int(_mem.noRoger))
